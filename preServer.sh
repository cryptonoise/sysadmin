...
# Принудительно привязываем ввод к терминалу, если скрипт запущен не в TTY
[[ -t 0 ]] || exec </dev/tty

# Функция безопасного чтения: очищает буфер stdin перед запросом
safe_read() {
    # Очищаем буфер ввода от остатков предыдущих команд
    while read -r -t 0 discard 2>/dev/null; do :; done
    # Выполняем обычный read с промптом
    read -r -p "$1" "$2"
}
...
# === Блок 7: Перезагрузка ===
# Проверяем, есть ли у нас терминал для вопроса
if [ -t 0 ]; then
