...
printf "\033[1;34m║  %-36s  ║\033[0m\n" "$SCRIPT_NAME"
printf "\033[1;34m║  Версия: %-29s  ║\033[0m\n" "$SCRIPT_VERSION"
printf "\033[1;34m║  %-36s  ║\033[0m\n" "$SCRIPT_DESC"
printf "\033[1;34m╚════════════════════════════════════════╝\033[0m\n"
...
SSH_KEY_INPUT=""
while true; do
    read -rp "Введите SSH публичный ключ (ssh-rsa ...): " SSH_KEY_INPUT
    ...
    # Базовая проверка формата (начинается с ssh-rsa, ssh-ed25519 и т.д.)
    if [[ "$SSH_KEY_INPUT" =~ ^(ssh-rsa|ssh-ed25519|ecdsa-sha2-nistp256|ecdsa-sha2-nistp384|ecdsa-sha2-nistp521)\  ]]; then
        break
    else
        printf "❌  Ошибка: Неверный формат ключа. Он должен начинаться с типа ключа (например, ssh-rsa).\n"
    fi
done
printf "✅  Ключ принят.\n"
echo "──────────────────────────────────────"
