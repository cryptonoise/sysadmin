...
# Дополнительно: удаляем файлы, которые после нейтрализации стали полностью пустыми
# или содержат только комментарии — это «мусорные» конфиги от облачных провайдеров,
# которые не несут полезной нагрузки и могут вызывать предупреждения sshd.
neutralize_sshd_dropins() {
    ...
    for f in /etc/ssh/sshd_config.d/*.conf; do
        ...
        # Если после нейтрализации файл содержит только комментарии/пустые строки — удаляем его
...
BOXART_TOP
        ...
        # ИСПРАВЛЕНИЕ: Символ ● (U+25CF) часто занимает 2 ячейки ширины в терминале.
        # Чтобы рамка не съезжала, мы компенсируем эту ширину, убирая лишние пробелы.
        # Было: 3 пробела + ●(2) + |(1) + ●(2) + 4 пробела = 12 ячеек (при 10 видимых символах) -> Сдвиг!
        # Стало: 2 пробела + ●(2) + |(1) + ●(2) + 3 пробела = 10 ячеек. Идеально ровно.
